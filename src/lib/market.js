...
//
// Today these are static files under /market/, generated by
// tools/build_market_data.py from the server's MySQL database and republished
// every half hour. When the Auction Tracker plugin ships it serves the same
// document shapes over HTTP, so pointing BASE at the plugin's origin is the
// whole migration - nothing below this line changes.
...
/** Axis labels on a one-day window, where every point shares a date. */
...
/** Matches on display name, material, category and enchantments, so "mending"
 *  finds the book, "pick" finds every pickaxe variant, and the auction house's
 *  own categories ("spawners", "ores") work as coarse filters. */
...
/**
 * Whether a market is a market at all.
 *
 * Admin tests and abandoned configurations sit in the same table as the real
 * pools, and one that holds nothing and has never traded is neither a price nor
 * a place to get one. Both figures have to be zero: a pool drained to empty
 * after real trading still belongs on the page.
 *
 * Filtered here rather than in the generator so the documents stay a faithful
 * dump of what the server has; this is a presentation call.
 */
export const poolIsReal = (pool) => pool.depth > 0 || pool.tradesAll > 0;

/** A real market that is quoting right now. */
